...
public class PersonalizeResults {
    public static void Execute(Tab tab) {
       ...
       if (tab != null && (tab.getUrl().getSpec().startsWith("chrome://")
                       ...
                       || tab.getUrl().getSpec().startsWith("nextkiwi://"))) {
          tab.getWebContents().evaluateJavaScript("(function() { if (!document.location.href.includes('chrome://') && !document.location.href.includes('chrome-extension://') && !document.location.href.includes('nextkiwi://')) { return; } " + ADAPT_TO_MOBILE_VIEWPORT + "})();", null);
       }
    ...
    private static boolean IsSearchUrl(String sUrl) {
        ...
        try {
          ...
          Log.w("NextKiwi", "MalformedURLException "+ e.getMessage());
        }
